...
export const Basket = ({ isBasketActive, setIsBasketActive }) => {
  const dishesInBasket = useSelector(
    (state) => state.reducerAddItem.basketItems
  );
  ...
  return (
    <>
      ...
      <div className={cl.content}>
          ...
          <div className={cl.basket}>
            {payment ? (
              <div className={cl.basket_info}>
                <div>
                  <h1>Підтвердження</h1>
                  <p className={cl.number}>Замовлення #12313</p>
                </div>
                <button
                  className={cl.btn_plus}
            ...
            <div
              className={payment ? cl.items_payment + " " + cl.items : cl.items}
            >
              {dishesInBasket.map((item) => (
                <BasketItem key={item.id} dish={item} />
              ))}
            </div>
        ...
        {payment && (
          <Payment payment={payment} value={value} setPayment={setPayment} />
        )}
      </div>
    </>
  );
